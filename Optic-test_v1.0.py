...
Instructions:  Capture the "show interfaces diagnostics optics" output in a log file, name the
log file "output.log" and place it in c:\.  Execute the script and it will return the data back.
...
for line in open("c:\\output.log", "r").read().splitlines():
